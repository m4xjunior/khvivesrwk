...
class FordService:
    ...
    def __init__(self):
        self.base_url = Config.FORD_API_URL
        self.cache = cache
        self.cache_prefix = "ford:"
    ...
    async def get_campaigns(self, vin: str):
        ...
        try:
            ...
            raise FordServiceError(f"Failed to fetch campaigns: {str(e)}")
    ...
    async def submit_validation(self, vin_id: str, status: int, observations: str):
        try:
            ...
            raise FordServiceError(f"Validation submission failed: {str(e)}")

class FordServiceError(Exception):
    pass
